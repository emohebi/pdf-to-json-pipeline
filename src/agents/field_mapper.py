...
class FieldMappingAgent:
    ...
    def _match_descriptions_via_llm(
        ...
        to its normalised value (only for matches above threshold).
...
Respond ONLY with a JSON array, no other text."""
        ...
        try:
            ...
            for m in matches:
                ...
                if (
                    0 <= idx < len(descriptions)
                    and 0 <= match_idx < len(reference_items)
                    and confidence >= threshold
                ):
                    original = descriptions[idx]
                    normalised = reference_items[match_idx]
                    result[original] = normalised
                    logger.debug(
            ...
            logger.info(
                f"[{document_id}] Description normalisation: "
                f"{len(result)}/{len(descriptions)} matched "
